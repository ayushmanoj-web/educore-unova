...
async function callOpenAI(chatMessages: any[]) {
...
async function callGemini(chatMessages: any[]) {
  ...
  const systemMessage = {
    parts: [{ text: 'You are Edudevadar AI, a concise, friendly study assistant for a school app. Give short, helpful answers.' }],
    role: 'user'
  };
...
serve(async (req) => {
  ...
  try {
    const { messages, prompt, model = 'chatgpt' } = await req.json();
    ...
    if (model === 'gemini') {
      ...
      generatedText = await callGemini(chatMessages);
    } else {
      if (!openAIApiKey) {
      ...
      generatedText = await callOpenAI(chatMessages);
    }
